...
class TaskManager {
    ...
    getLevelsConfig() {
        return {
            1: { name: 'Level 1 Member', minDeposit: 200, dailyTasks: 65, commissionRate: 0.6, earning: '$7-12/day' },
            2: { name: 'Level 2 Member', minDeposit: 1800, dailyTasks: 85, commissionRate: 1, earning: '$50-60/day' },
            3: { name: 'Level 3 Member', minDeposit: 3500, dailyTasks: 100, commissionRate: 3, earning: '$300-500/day' },
            4: { name: 'Level 4 Member', minDeposit: 4500, dailyTasks: 140, commissionRate: 5, earning: '$400-500/day' },
            5: { name: 'Level 5 Member', minDeposit: 8000, dailyTasks: 160, commissionRate: 8, earning: '$500-600/day' }
        };
    }
        ...
        document.getElementById('current-task').innerHTML = `
            <div id="task-fade" style="opacity:0; transition: opacity 0.7s;">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
                    <h3 class="text-lg sm:text-xl font-bold">Transfer Order #${task.id}</h3>
                    <div class="flex items-center ${task.urgency === 'high' ? 'text-red-400' : 'text-blue-400'} self-start sm:self-auto">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6">
                    <!-- Sender Info -->
                    <div class="bg-gradient-to-r from-green-600/10 to-green-800/10 backdrop-blur-sm rounded-xl p-4 sm:p-5 border border-green-500/20">
                        <h4 class="font-semibold mb-4 text-green-400 flex items-center">
                        ...
                        <div class="space-y-3">
                            <div class="flex flex-col sm:flex-row sm:justify-between gap-1">
                                ...
                                <span class="font-semibold text-white">${task.customerName}</span>
                            </div>
                            <div class="flex flex-col sm:flex-row sm:justify-between gap-1">
                                <span class="text-gray-300 text-sm">Amount:</span>
                                <span class="font-semibold text-white text-lg">${this.formatCurrency(task.amount, task.fromCurrency)}</span>
                            </div>
                            <div class="flex flex-col sm:flex-row sm:justify-between gap-1">
                                <span class="text-gray-300 text-sm">Country:</span>
                                <span class="font-semibold text-white">${this.getCountryName(task.fromCurrency)}</span>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Receiver Info -->
                    <div class="bg-gradient-to-r from-blue-600/10 to-blue-800/10 backdrop-blur-sm rounded-xl p-4 sm:p-5 border border-blue-500/20">
                        <h4 class="font-semibold mb-4 text-blue-400 flex items-center">
                        ...
                        <div class="space-y-3">
                            <div class="flex flex-col sm:flex-row sm:justify-between gap-1">
                                <span class="text-gray-300 text-sm">Name:</span>
                                <span class="font-semibold text-white">${task.receiverName}</span>
                            </div>
                            <div class="flex flex-col sm:flex-row sm:justify-between gap-1">
                                <span class="text-gray-300 text-sm">Receives:</span>
                                <span class="font-semibold text-white text-lg">${this.formatCurrency(task.convertedAmount, task.toCurrency)}</span>
                            </div>
                            <div class="flex flex-col sm:flex-row sm:justify-between gap-1">
                                <span class="text-gray-300 text-sm">Country:</span>
                                <span class="font-semibold text-white">${this.getCountryName(task.toCurrency)}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-4 bg-gradient-to-r from-purple-600/15 to-blue-600/15 backdrop-blur-sm rounded-xl p-4 sm:p-5 border border-purple-500/30">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                        <span class="text-gray-300 font-medium">Exchange Rate:</span>
                        <span class="text-lg sm:text-xl font-bold text-purple-400">1 ${task.fromCurrency} = ${this.formatCurrency(task.rate, task.toCurrency).replace(/[^\d.,]+/, '')} ${task.toCurrency}</span>
                    </div>
                </div>
                
                <!-- Commission Preview -->
                <div class="mb-6 bg-gradient-to-r from-green-600/15 to-emerald-600/15 backdrop-blur-sm rounded-xl p-4 sm:p-5 border border-green-500/30">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                        <span class="text-gray-300 font-medium">Your Commission (${this.getLevelsConfig()[this.userData.currentLevel].commissionRate}%):</span>
                        <span class="text-xl sm:text-2xl font-bold text-green-400">${this.formatCurrency(this.calculateCommissionInUSD(task), 'USD')}</span>
                    </div>
                </div>
                ...
                <div class="flex">
                    <button 
                        ...
                        class="btn-primary w-full py-4 text-base sm:text-lg font-semibold touch-manipulation"
                        ${(!hasSufficientDeposit || dailyTasksCompleted) ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : ''}
                    >
                        <span id="complete-transfer-btn-text" class="flex items-center justify-center">
                ...
                <div id="confirm-modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 hidden">
                    <div class="bg-gray-900 rounded-xl p-6 sm:p-8 max-w-sm w-full mx-3 text-center">
                        <h3 class="text-lg sm:text-xl font-bold mb-4">Confirm Transfer</h3>
                        <p class="mb-6 text-sm sm:text-base">Are you sure you want to complete this transfer?<br><span class='text-green-400 font-bold'>${this.formatCurrency(task.amount, task.fromCurrency)}</span> → <span class='text-blue-400 font-bold'>${this.formatCurrency(task.convertedAmount, task.toCurrency)}</span></p>
                        <div class="flex justify-center gap-3 sm:gap-4">
                            <button id="confirm-transfer-btn" class="btn-primary px-4 py-2 text-sm sm:text-base touch-manipulation">Yes, Complete</button>
                            <button id="cancel-transfer-btn" class="btn-secondary px-4 py-2 text-sm sm:text-base touch-manipulation">Cancel</button>
                        </div>
                    </div>
                </div>
                <!-- Loading Modal -->
    ...
    addToTaskHistory(task, commission) {
        ...
        taskElement.innerHTML = `
            <div class="flex justify-between items-center">
                <div>
                    <div class="font-semibold">${task.fromCurrency}→${task.toCurrency}</div>
